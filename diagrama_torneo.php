...
foreach ($pdo->query($sqltorn) as $row) {
	...
	for ($i = 1; $i < 8; $i++) {
		...
		<div class="container">
			...
			<button class = "elim" onclick = "eliminar('.$row["id_torneo"].')">Eliminar torneo</button>
			<div class="header">
...
<script>
  ...
  function toFotos() {
	...
	if (isAdmin) {
		document.querySelector(".masTorneo").style.display = "block";
		document.querySelectorAll(".agregar").forEach(element => {
			element.style.display = "block";
		});

		document.querySelectorAll(".elim").forEach(element => {
			element.style.display = "block";
    ...
    console.log("hola");
    document.querySelector("#formTorneo").innerHTML = '<input type="hidden" value= 0 name = "isAdmin">';
    document.querySelector("#formTorneo").submit();
  }
...
function eliminar(id) {
	var xhr = new XMLHttpRequest();
        xhr.open("POST", "delete_process.php", true); // Specify the request type and URL
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"); // Set the request header for form data
        xhr.onreadystatechange = function() {
          if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
          }
        };
        xhr.send("torneo=" + encodeURIComponent(id)); // Send the data to the server
